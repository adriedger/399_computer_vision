...
from a7_2 import get3Dcube, project, getTransformMatrices

# geths n_pts feature points by clicking on image img
def getFeaturePoints(img, n_pts, pt_size=5, col=(255, 255, 255), title=None):
    if title is None:
...
# main stereo function 
# x1,x2 - image coords of feature points that we recosntruct
# B - baseline
# f - focal length 
# cx , cy - center of the image 
def stereo(f, B, x1, x2, y1, cx, cy):
    # get Z from x1,x2,B,f
    # using formula Z = f*B/(x1-x2)
    Z = f*8/(x1-x2)
    Z = Z.reshape(-1,1)

    # recosntruct 3D coordinates in the 1st camera coordinate system
    # X coordinate from x1; Y coordinates from y1
    #     change image coord system to the center of the image (you know (cx,cy))
    #     change units from pixels to 3D world units (you know Z and f) 
    # X,Y should be colum vectors 
    X = (x1-cx).reshape(-1,1)
    Y = (y1-cy).reshape(-1,1)
    scale = (Z/f).reshape(-1,1)
    X = (X*scale).reshape(-1,1)
    Y = (Y*scale).reshape(-1,1)

    # concatenate to P - should be size n_ptsx3
...
def main():
    ...
    baseline = 0.5 # camera translation between the stereo images
    ...
    # second image ; translation along baseline ; figure out translation
